...
export default class Shop2 extends Scene {
  ...
  public override render(canvas: HTMLCanvasElement): void {
    ...
    if (this.textScene) {
      const lines: string[] = ['Verify sender\'s email against official domain for legitimacy.', 'Trustworthy emails have quality content: watch for errors and urgent requests.', 'Be cautious of phishing emails, verify unexpected ones through official channels.', 'Hover over links to preview URLs, ensuring alignment with context.', '"https://" signifies a secure website; avoid personal info on "http://" sites.', ' Check link sources, especially in emails, to verify legitimacy.', ' Secure sites display padlock and "https"; verify before sharing sensitive data.', 'Verify clear contact details on trustworthy websites before engagement.', 'Check user reviews before trusting a website; caution with negative reviews.', 'Exercise caution with sites lacking online presence or positive recommendations.', 'Press the F Key']; // Voeg elke regel tekst toe aan dit array
